...
// ";
            ...
            $query="select * from student_scholarship where usn='$usn'";
           $result = mysqli_query($conn,$query) or die("QUERRY UNSUCESSFULL");
           $quer = "SELECT * FROM scholarship_details";
...
<head>
    ...
    <link href="registration1.css" rel="stylesheet" type="text/css"/>
            ...
            if(mysqli_num_rows($result) > 0){
               while( $row = mysqli_fetch_assoc($result))
               {
                ...
                while($row2 = mysqli_fetch_assoc($res)){
                    if($row2['scholarship_id'] == $row['scholarship_id'])
                    {
                        $name = $row2['scholarship_name'];
                    }
                }?>
                   <tr>
                    <td><?php echo $name;?></td>
